...
VENV_CLEAR_FLAG=""
if [ -d "$VENV_PATH" ]; then
    echo "Existing .venv detected. Recreating it non-interactively..."
    VENV_CLEAR_FLAG="--clear"
fi
"$UV_CMD" venv "$VENV_PATH" --python 3.12 $VENV_CLEAR_FLAG

# 3. Install Dependencies
echo "[3/4] Installing SDK from requirements.txt..."
